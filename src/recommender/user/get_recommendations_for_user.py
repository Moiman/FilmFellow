import numpy as np
from .restrict_favourites import restrict_favourites
from .top_rated import top_rated
from .collaborative_filtering_user import collaborative_filtering_user
from .content_based_filtering_user import content_based_filtering_user
from .get_recommendations_for_all_ratings import \
    get_recommendations_for_all_ratings
import scipy.sparse
import random

def get_recommendations_for_user(ratings, favourites):
    """
    Gives recommendations for a user with movie ratings and favourited movies.
    
    Args:
        ratings (dict: int:float64): The given ratings as TMDB id: rating value 
        pairs.
    ...
    TMDB_to_MovieLens = np.load(
        "user/TMDB_to_MovieLens.npy", 
        allow_pickle=True).item()
    movie_id_to_index_con = np.load(
        "user/content_based_filtering/movie_id_to_index.npy", 
        allow_pickle=True).item()
    movie_index_to_id_con = np.load(
        "user/content_based_filtering/movie_index_to_id.npy", 
        allow_pickle=True).item()
    movie_id_to_index_coll = np.load(
        "user/collaborative_filtering/movie_mapper.npy", 
        allow_pickle=True).item()
    movie_index_to_id_coll = np.load(
        "user/collaborative_filtering/movie_inverse_mapper.npy", 
        allow_pickle=True).item()
    matrix = scipy.sparse.load_npz("user/collaborative_filtering/sparse_matrix.npz")
    
    num_movies_in_cosine_sim = 1841
    cosine_sim = np.memmap(
       "user/content_based_filtering/cosine_sim_memmap.npy", 
       dtype="float64", 
       mode='r', 
