...
def content_based_new_movie(movie_features, 
    ...
    Args:
        movie_features (dict: str): Features present in the movie: genres 
        and tags.
    ...
    cosine_sim_new = np.memmap(
       "content_based_filtering/cosine_sim_new_memmap.npy", 
       dtype="float64", 
       mode='r+', 
       shape=(num_movies_in_cosine_sim, num_movies_in_cosine_sim))
    
    matrix = pd.read_csv("content_based_filtering/movies_matrix.csv")
